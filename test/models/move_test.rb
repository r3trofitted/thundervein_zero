...
class MoveTest < ActiveSupport::TestCase
  test "a player cannot declare a move from a hex they don't occupy" do
  ...
  test "a player can move to an empty hex" do
    move = Move.new(turn: @new_game_turn_1, player: @steve, origin: :south, target: :east, units: 1)
    assert move.valid?
  end
  
  test "a player can move to a hex they already occupy" do
    move = Move.new(turn: @ongoing_game_turn_3, player: @noemie, origin: :west, target: :north, units: 1)
    assert move.valid?
  end
  
  test "a player cannot move to a hex already occupied by another player" do
  ...
  test "a player can not move to a hex that is not adjacent to the target hex" do
  ...
  test "resolving a move" do
    ...
    assert_equal [Board::Hex.new(occupant: @steve, units: 1), :origin], board_updates[:south]
    assert_equal [Board::Hex.new(occupant: @steve, units: 2), :target], board_updates[:east]
  end
end
